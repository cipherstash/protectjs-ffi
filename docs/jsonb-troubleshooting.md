...
**Important:** When using @cipherstash/stack with `{ path: "user.role", value: "admin" }`, this is converted to a containment query (`{ user: { role: "admin" } }`) with `sv` output, **not** a selector+value comparison.
